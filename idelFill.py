...
def uploadProcess( minimal_framesize, estimatingType, pTrackUsed, pBufferTime):
    ...
    for singleFrame in range( howLongIsVideoInSeconds * FPS ):
        ...
        if (singleFrame % (5 * FPS) == 0 and ( estimatingType == "ProbabilityPredict" or estimatingType == "Marginal") ):
        ...
        while True: 
            ...
            if (innerCount == 0):
                if (estimatingType == "ProbabilityPredict"):
                    ...
                    if (len(lookbackwardHistogramS)>100):
                        ...
                        quantValue = quantile(decision_list, pEpsilon)
                        suggestedFrameSize = quantValue
                        # pyplot.xlim([np.percentile(decision_list,0), np.percentile(decision_list,95)]) 
                        # pyplot.hist(decision_list, bins=100)
                        # pyplot.axvline(x=quantValue, color = "black")
                        # pyplot.show()
                ...
                thisFrameSize = minimal_framesize
            ...
            if (singleFrame < len(frame_prepared_time)-1  and  runningTime >= frame_prepared_time[ singleFrame+1 ]):
                break
