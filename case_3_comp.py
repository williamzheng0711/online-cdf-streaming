...
def uploadProcess( minimal_framesize, estimatingType, pTrackUsed, pBufferTime):
    ...
    for singleFrame in range( howLongIsVideoInSeconds * FPS ):
        ...
        if (estimatingType == "ProbabilityPredict" or "onlineRLS"):  
        ...
        # elif estimatingType == "onlineRLS":
        #     if (singleFrame % 100 == 99):
        #         print("Frame: " + str(singleFrame) + " .Now is time: " + str(runningTime))
        ...
        if runningTime > frame_prepared_times[singleFrame] + 1/FPS + pBufferTime:
            ...
            if now_go_real and np.abs(frame_prepared_times[singleFrame] + 1/FPS + pBufferTime - runningTime) > 0.00001:
                print("有點問題" + str(runningTime) + " the last DDL =" + str(frame_prepared_times[singleFrame] + 1/FPS + pBufferTime))
        ...
        lastX = np.array(np.array(realVideoFrameSize[-5:])/np.array(transmitHistoryTimeLog[-5:])) if (len(realVideoFrameSize)>=5 and len(transmitHistoryTimeLog)>=5) else np.ones(5)
        ...
        if (estimatingType == "ProbabilityPredict" or "onlineRLS"):
            ...
            if (estimatingType == "onlineRLS"):
                ...
                # print(c_avg_new_hat, "thisisis")
                error_quantile = np.quantile(errors[-1000:], pEpsilon, method="median_unbiased") if len(errors)>0 else 0
        ...
        filt.adapt(thisFrameSize / uploadDuration, lastX)
